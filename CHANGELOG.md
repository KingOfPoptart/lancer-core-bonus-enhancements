...
- Pins are stored in an actor flag keyed by a mount signature (type + fitting
  sizes + index), so they survive most loadout edits.
...
- Mount-signature keying can lose a pin if you have two identically configured
  mounts and delete one. Signature reconciliation is a planned improvement.
